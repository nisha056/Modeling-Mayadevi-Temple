...
void Render::getModels()
{
    for (int i = 0; i < temple->children.size(); ++i)
    {
        modelPosition.push_back(temple->children[i]->getPosition());
        modelScale.push_back(temple->children[i]->getScale());
        modelAngle.push_back(temple->children[i]->getAngle());
        modelRotationVector.push_back(temple->children[i]->getRotationVector());
        Model model(temple->children[i]->getModelName());
...
void Render::initializeGlfw()
{
    ...
    //Create glfw window
    window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "Mayadevi Temple visualization", NULL, NULL);
    if (window == NULL)
    {
